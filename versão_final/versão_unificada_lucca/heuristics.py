...
def heuristic_defensive_structures(state, player):
    ...
    for r in range(state.size):
        for c in range(state.size):
            if state.board[r][c] == player:
                score += 10 # Material
                # Player 1 quer aumentar R, Player 2 quer diminuir R
                score += r if player == 1 else (state.size - 1 - r)
                # Checa se tem suporte de uma peça amiga atrás
                back_r = r - 1 if player == 1 else r + 1
                ...
                # Penalidade pelo avanço do oponente
                score -= r if opponent == 1 else (state.size - 1 - r)

                # Lógica inversa para o oponente (suporte dele)
                back_r_opp = r - 1 if opponent == 1 else r + 1
